...
`bgg sync [--full]`, `bgg list [--owned] [--json]`, `bgg status` (default).
...
- **Credential storage:** OS keyring via the `keyring` crate. Store the **cookies**, not the password. On 401, prompt for password and re-login.
...
- **The only network call:** `GET https://boardgamegeek.com/xmlapi2/collection?username=<name>&stats=1`. Use `modifiedsince=YY-MM-DD HH:MM:SS` (with a 1-minute safety margin) for incremental sync. Handle `202 Accepted` by retrying with backoff.
- **Rate limiting:** BGG asks for a 5-second minimum between requests. Enforced in `HttpClient` via a `Mutex<Instant>` even though one sync = one call.
